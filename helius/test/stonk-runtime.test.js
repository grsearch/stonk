...
test('CPMM account decoder and adapter use mint precision, effective reserves, and SOL valuation', async () => {
  ...
  await assert.rejects(adapter.prepare({ ...p, graduatedAt: now - 14400000 }));
...
test('Shadow four-hour cutoff censors proxy entry and all recovery arms without a fictitious close', () => {
  const at = Date.now(), events = [], tracker = new Tracker(options(c), r => events.push(r), { now: () => at });
  const graduatedAt = at - 14399000;
