...
当前 Stonk 参数：砸单至少 7 SOL、跌幅 10%–30%、报价储备至少 30 SOL、服务器当前每笔模拟 0.1 SOL（安装示例同步为 0.1）、最多 20 仓、止盈 20%、止损 25%、上涨 10% 后回撤 3% 退出、最长持有 20 秒（每秒巡检触发超时退出）。同币主模拟亏损后冷却 60 秒，Shadow 冷却对照同为 60 秒；普通入场冷却仍为 30 秒。未启用的实盘策略配置也使用 7 SOL 门槛、60 秒亏损冷却和 20 秒持仓上限，实盘执行器仍被禁用。
...
股票报价 mint 支持已核验的 Token-2022 发行者控制、未启用的转账钩子、未暂停状态及 Scaled UI Amount 等扩展。内部全程使用原始整数余额和 decimals，UI 倍率不重复计入估值；仍计入公开转账费。已启用的转账钩子、暂停、未知或损坏扩展仍拒绝。此支持只针对报价资产的只读模拟，不代表获得发行者授权或具备可执行兑换路径，本币的扩展限制仍保留。
...
在服务器交互终端执行 `sudo python3 /opt/stonk-monitor/deploy/update-wallet.py`，按提示输入 Base58 格式的 Solana 钱包私钥。输入隐藏，只输出校验后的钱包地址。工具仅更新 `.env` 并限制权限为 600，不重启、不启用交易。当前 Stonk 仍只有 PaperExecutor；预留实盘参数不代表真实 Raydium 执行链路已经实现。
